...
class Ball {
  ...
  show() {
	...
	reflect() {
		//wand in +x richtung
		if(this.pos.x+this.d/2 > width - 1.5*SpielfeldRand) {
			this.pos.x = width - this.d/2 - 1.5*SpielfeldRand;
			this.vel.x = this.vel.x * (-1);
			a = 2*a;
			this.c = color(random(0,255),random(0,255),random(0,255));
		}
		//wand in -x richtung
		if(this.pos.x-this.d/2 < 1.5*SpielfeldRand) {
			this.pos.x = this.d/2 + 1.5*SpielfeldRand;
			this.vel.x = this.vel.x * (-1);
			a = 2*a;
			this.c = color(random(0,255),random(0,255),random(0,255));
		}
		//wand in +y richtung
		if(this.pos.y+this.d/2 > height - 1.5*SpielfeldRand) {
			this.pos.y = height - this.d/2 - 1.5*SpielfeldRand;
			this.vel.y = this.vel.y * (-1);
			a = 2*a;
			this.c = color(random(0,255),random(0,255),random(0,255));
		}
		//wand in -y richtung
		if(this.pos.y-this.d/2 < 1.5*SpielfeldRand) {
			this.pos.y = this.d/2 + 1.5*SpielfeldRand;
			this.vel.y = this.vel.y * (-1);
			a = 2*a;
			this.c = color(random(0,255),random(0,255),random(0,255));
		}
	...
	decideCollision(otherBall) {
		...
		//sind Bälle ineinander werden sie entlang otherDirection verschoben, damit sie nicht mehr in ineinander stecken
	...
	// draw an arrow for a vector at a given base (vector)position
